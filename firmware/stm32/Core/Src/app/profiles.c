...
/* Core/Src/app/profiles.c — ПУСТАЯ ВЕРСИЯ
 *
 * Хардкодных профилей нет — приходят только с сервера.
 * Меняется ТОЛЬКО этот файл, остальное (скринсейвер, слайды,
 * App_Init/App_Update) не трогаем.
 *
 * Строки пустые "" (не NULL) — чтобы App_Init спокойно отработал
 * на пустом устройстве до первого обновления.
 *
 * Профили живут в RAM, после ребута пропадают — для теста это норма.
 */
#include "profiles.h"
#include "config.h"
#include <stdint.h>
...
Profile_t profiles[NUM_PROFILES] = {
    { s_empty, s_empty, 0x00, 0x0000, s_empty, NULL, 0, 0, 0 },
    { s_empty, s_empty, 0x00, 0x0000, s_empty, NULL, 0, 0, 0 },
    { s_empty, s_empty, 0x00, 0x0000, s_empty, NULL, 0, 0, 0 },
    { s_empty, s_empty, 0x00, 0x0000, s_empty, NULL, 0, 0, 0 },
    { s_empty, s_empty, 0x00, 0x0000, s_empty, NULL, 0, 0, 0 },
